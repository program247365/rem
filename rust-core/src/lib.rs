...
mod tests {
    ...
    fn test_search_state_basic_operations() {
        ...
        assert!(!tui_app.is_search_active(), "Search should be inactive initially");
        assert_eq!(tui_app.get_search_query(), "", "Search query should be empty initially");
        
        // Test starting global search
        tui_app.start_global_search();
        assert!(tui_app.is_search_active(), "Search should be active after starting");
        assert!(tui_app.is_global_search(), "Should be in global search mode");
        ...
        tui_app.add_search_char('t');
        assert_eq!(tui_app.get_search_query(), "test", "Search query should be 'test'");
        
        // Test backspace
        tui_app.remove_search_char();
        assert_eq!(tui_app.get_search_query(), "tes", "Search query should be 'tes' after backspace");
        
        // Test clearing search
        tui_app.clear_search();
        assert!(!tui_app.is_search_active(), "Search should be inactive after clearing");
        assert_eq!(tui_app.get_search_query(), "", "Search query should be empty after clearing");
    ...
    fn test_search_filtering() {
        ...
        assert_eq!(buy_results.len(), 1, "Should find 1 reminder with 'buy' (only incomplete by default)");
        assert_eq!(buy_results[0].title, "Buy groceries", "Should find the incomplete 'Buy groceries' reminder");
        
        // Enable completed todos and search again
        tui_app.set_show_completed_todos(true);
        let buy_results_with_completed = tui_app.get_filtered_reminders_for_test();
        assert_eq!(buy_results_with_completed.len(), 2, "Should find 2 reminders with 'buy' when showing completed");
        ...
        assert_eq!(appointment_results.len(), 1, "Should find 1 reminder with 'appointment' in notes");
        assert_eq!(appointment_results[0].title, "Call dentist", "Should find the 'Call dentist' reminder");
        ...
        assert_eq!(exercise_results.len(), 1, "Should find 1 reminder with case insensitive 'EXER' search");
        assert_eq!(exercise_results[0].title, "Exercise", "Should find the 'Exercise' reminder");
    ...
    fn test_search_modes() {
        ...
        tui_app.add_search_char('t');
        tui_app.exit_search();
        assert!(!tui_app.is_search_active(), "Search should be inactive after exit");
        assert!(tui_app.has_search_results(), "Should still have search results");
        assert_eq!(tui_app.get_search_query(), "test", "Search query should be preserved");
        
        // Test clear search (removes everything)
        tui_app.clear_search();
        assert!(!tui_app.is_search_active(), "Search should be inactive after clear");
        assert!(!tui_app.has_search_results(), "Should not have search results after clear");
        assert_eq!(tui_app.get_search_query(), "", "Search query should be empty after clear");
    ...
    fn test_global_search_action() {
        ...
        let action = TuiAction::GlobalSearch { query: "test query".to_string() };
        
        match action {
            TuiAction::GlobalSearch { query } => {
                assert_eq!(query, "test query", "GlobalSearch action should contain the correct query");
            }
            _ => {
    ...
    fn test_search_with_empty_query() {
        ...
        tui_app.start_list_search();
        assert!(tui_app.is_search_active(), "Search should be active");
        assert!(!tui_app.has_search_results(), "Should not have results with empty query");
        ...
        assert_eq!(all_results.len(), 2, "Should show all reminders with empty search query");
        
        // Add character then remove it
        tui_app.add_search_char('t');
        assert!(tui_app.has_search_results(), "Should have results after adding character");
        
        tui_app.remove_search_char();
        assert!(!tui_app.has_search_results(), "Should not have results after removing character");
    ...
    fn test_search_filtering_combination() {
        ...
        assert_eq!(one_results_incomplete_only.len(), 1, "Should find 1 incomplete reminder with 'one'");
        assert_eq!(one_results_incomplete_only[0].title, "Task one incomplete", "Should find the incomplete task");
        
        // Show completed todos and search again
        tui_app.set_show_completed_todos(true);
        let one_results_all = tui_app.get_filtered_reminders_for_test();
        assert_eq!(one_results_all.len(), 2, "Should find 2 reminders with 'one' when showing completed");
        
        // Verify both completed and incomplete are found
        let titles: Vec<&str> = one_results_all.iter().map(|r| r.title.as_str()).collect();
        assert!(titles.contains(&"Task one complete"), "Should contain completed task");
        assert!(titles.contains(&"Task one incomplete"), "Should contain incomplete task");
        
        // Hide completed again and verify filtering
        tui_app.set_show_completed_todos(false);
        let one_results_incomplete_again = tui_app.get_filtered_reminders_for_test();
        assert_eq!(one_results_incomplete_again.len(), 1, "Should find 1 incomplete reminder again");
    ...
    fn test_global_search_view_switching() {
        ...
        assert!(matches!(tui_app.get_current_view(), AppView::Lists), "Should start in Lists view");
        ...
        tui_app.set_current_view(AppView::Reminders { list_id: "global".to_string() });
        
        // Verify view switched
        assert!(tui_app.is_in_global_search_view(), "Should be in global search view");
    ...
    fn test_global_search_data_handling() {
        ...
        let all_filtered = tui_app.get_filtered_reminders_for_test();
        assert_eq!(all_filtered.len(), 2, "Should show 2 incomplete reminders by default");
        ...
        assert_eq!(work_filtered[0].title, "Work task", "Should find the work task");
        
        // Test that completed todos can be shown in global search
        tui_app.set_show_completed_todos(true);
        let work_with_completed = tui_app.get_filtered_reminders_for_test();
        assert_eq!(work_with_completed.len(), 1, "Should still find 1 reminder with 'work' including completed");
        ...
        assert_eq!(shop_filtered.len(), 1, "Should find completed shopping task");
        assert_eq!(shop_filtered[0].title, "Shopping list", "Should find the shopping task");
    ...
    fn test_global_search_list_name_lookup() {
        ...
        assert_eq!(list_name_1, Some("Work List"), "Should find list name for rem-1");
        
        let list_name_2 = tui_app.get_list_name_for_reminder("rem-2");
        assert_eq!(list_name_2, Some("Personal List"), "Should find list name for rem-2");
        
        let list_name_none = tui_app.get_list_name_for_reminder("nonexistent");
        assert_eq!(list_name_none, None, "Should return None for nonexistent reminder");
    ...
    fn test_global_search_escape_behavior() {
        ...
        tui_app.exit_search();
        assert!(!tui_app.is_search_active(), "Search should not be active after exit");
        assert!(tui_app.has_search_results(), "Should still have search results after exit");
        assert_eq!(tui_app.get_search_query(), "test", "Search query should be preserved after exit");
        
        // Test clear search (second escape equivalent)
        tui_app.clear_search();
        assert!(!tui_app.is_search_active(), "Search should not be active after clear");
        assert!(!tui_app.has_search_results(), "Should not have search results after clear");
        assert_eq!(tui_app.get_search_query(), "", "Search query should be empty after clear");
        assert!(!tui_app.is_global_search(), "Should not be in global search mode after clear");
    ...
    fn test_global_search_integration() {
        ...
        assert_eq!(initial_reminders.len(), 2, "Should show 2 incomplete reminders initially");
        ...
        assert_eq!(buy_results[0].title, "Buy groceries", "Should find the groceries reminder");
        ...
        assert_eq!(meeting_results.len(), 1, "Should find 1 reminder with 'meet' in notes");
        assert_eq!(meeting_results[0].title, "Prepare presentation", "Should find the presentation reminder");
        ...
        assert_eq!(review_results[0].title, "Review code", "Should find the review reminder");
        assert!(review_results[0].completed, "Found reminder should be completed");
    ...
    fn test_global_search_navigation_back() {
        ...
        assert!(!tui_app.is_global_search(), "Should not be in global search mode");
        ...
        assert!(matches!(tui_app.get_current_view(), AppView::Lists), "Should be back in Lists view");
        assert!(!tui_app.is_search_active(), "Search should be cleared");
